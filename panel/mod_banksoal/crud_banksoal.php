...
if ($pg == 'simpan_soal') {
    ...
    if ($jenis == '1') {
        ...
        if ($_SESSION['editor'] == true) {
            ...
            $pilA = addslashes(htmlspecialchars($_POST['pilA']));
            $pilB = addslashes(htmlspecialchars($_POST['pilB']));
            $pilC = addslashes(htmlspecialchars($_POST['pilC']));
            $pilD = addslashes(htmlspecialchars($_POST['pilD']));
            $pilE = addslashes(htmlspecialchars($_POST['pilE']));
            $kategori = $_POST['kategori'];
        }
        ...
        if ($jumsoal == 0) {
            $exec = mysqli_query($koneksi, "INSERT INTO soal (kategori,id_mapel,nomor,soal,jenis,pilA,pilB,pilC,pilD,pilE,jawaban,file,file1,fileA,fileB,fileC,fileD,fileE) VALUES ('$kategori','$id_mapel','$nomor','$isi_soal','1','$pilA','$pilB','$pilC','$pilD','$pilE','$jawaban','$urlx','$filex1','$filexA','$filexB','$filexC','$filexD','$filexE')");
        } else {
            $exec = mysqli_query($koneksi, "UPDATE soal SET kategori='$kategori',soal='$isi_soal',pilA='$pilA',pilB='$pilB',pilC='$pilC',pilD='$pilD',pilE='$pilE',jawaban='$jawaban',file='$urlx',file1='$filex1',fileA='$filexA',fileB='$filexB',fileC='$filexC',fileD='$filexD',fileE='$filexE' WHERE id_mapel='$id_mapel' AND nomor='$nomor' AND jenis='1'");
        }
    }
    if ($jenis == '2') {
        if (isset($_FILES['file']['name']) && $_FILES['file']['name'] <> '') {
